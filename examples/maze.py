...
from vmflib2.games import tf as tf2
import numpy
...
m.add_solids(blocks + [floor] + [ceiling])

# Define RED spawn
spawn_red = tf2.InfoPlayerTeamspawn(m, TeamNum=2, angles="0 45 0")
spawn_red.origin = "%s %s %s" % ((-maze_width / 2) + (maze_block_size * 1.5),
                                 (-maze_height / 2) + (maze_block_size * 1.5),
                                 0)

# Define BLU spawn
spawn_blu = tf2.InfoPlayerTeamspawn(m, TeamNum=3, angles="0 -135 0")
